tailwind.config = {
    ...
    const kelasConfig = {
      className: 'IX-B',
      wali: 'Ferri Muhammad',
      slogan: '"Sedikit-sedikit jadi bukit, ini agit boss."',
      jumlahSiswa: 10
    };
...
// ===== RENDER KAS (READ dari Supabase) =====
    ...
    async function loadKasList() {
      ...
      try {
        var result = await supabaseFetchWithRetry(function() {
          return supabase.from('kas_ixb').select('*').order('created_at', { ascending: false });
        }, 5000, 3);
        var data = result.data;
        var error = result.error;
    ...
    async function addKasTransaction(tipe, jumlah, keterangan) {
      ...
      const { data, error } = await supabase.from('kas_ixb').insert(payload).select();
...
// ============================================================
    ...
    // keep old reference for CRUD, but route rendering through new UI
    async function loadJadwalFromDB() {
      await fetchAndRenderJadwal(kelasConfig.className);
    }
    const prData = [];
